...
public class Menger {
    ...
    public string[] organize() {
        ...
        foreach (string[] entry in dataListSplit) {
            ...
            entriesByLevel[entries.Length - 1] += entrySeparator + entry;
        }
        foreach (string entries in entriesByLevel) {
            entriesOfLayer = alphabeticallySort(entries.Split(entrySeparator));
            foreach (string entry in entriesOfLayer) {
